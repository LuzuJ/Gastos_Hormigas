...
export const PaymentSourceModal: React.FC<PaymentSourceModalProps> = ({
  ...
  useEffect(() => {
    if (source) {
      ...
      // Si el color no está en los tipos predefinidos, es un color personalizado
      const defaultTypeColor = PAYMENT_SOURCE_TYPES.find(t => t.value === source.type)?.color;
      if (source.color && source.color !== defaultTypeColor) {
  ...
  const finalColor = customColor || selectedTypeInfo?.color || '#6B7280';
  const finalIcon = selectedTypeInfo?.icon || '📝';
  ...
  const handleTypeChange = (newType: PaymentSourceType) => {
    setType(newType);
    // Limpiar color personalizado al cambiar tipo
    setCustomColor('');
  };
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <div className={styles.formGroup}>
            <span className={styles.label}>
              Ícono y Color <span className={styles.required}>*</span>
            </span>
            
            {/* Selección de íconos por tipo */}
            <div className={styles.subSection}>
              <span className={styles.subLabel}>Selecciona un ícono:</span>
              <div className={styles.typeGrid}>
                {PAYMENT_SOURCE_TYPES.map((typeOption) => (
                  <button
                    ...
                    style={{
                      borderColor: type === typeOption.value ? finalColor : undefined
                    }}
                  >
                    <span 
                      className={styles.typeIcon}
                      style={{ color: type === typeOption.value ? finalColor : typeOption.color }}
                    >
                      {typeOption.icon}
                    </span>
            ...
            <div className={styles.subSection}>
              <span className={styles.subLabel}>Selecciona un color:</span>
              <div className={styles.colorPalette}>
                {COLOR_PALETTE.map((color) => (
                  <button
                    ...
                    className={`${styles.colorOption} ${customColor === color ? styles.selectedColor : ''}`}
                    style={{
                      backgroundColor: color,
                      borderColor: customColor === color ? color : 'transparent'
                    }}
                    title={color}
                  />
                ))}
