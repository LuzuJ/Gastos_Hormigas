...
export const EditExpenseModal: React.FC<EditModalProps> = ({ expense, categories, onClose, onSave, onAddSubCategory }) => {
    ...
    useEffect(() => {
        ...
        const initialSubcategories = initialCategory?.subcategories || [];
        setAvailableSubCategories(initialSubcategories);

        const subCategoryExists = initialSubcategories.some(s => s.name === expense.subCategory);
        if (initialSubcategories.length === 0 || !subCategoryExists) {
            setShowNewSubCategoryInput(true);
            setFormData(prev => ({...prev, subCategory: ADD_NEW_SUBCATEGORY_VALUE}));
        }
    }, [expense, categories]);
    ...
    const handleCategoryChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        const newSubCategory = subcategories[0]?.name || ADD_NEW_SUBCATEGORY_VALUE;
        ...
        setShowNewSubCategoryInput(newSubCategory === ADD_NEW_SUBCATEGORY_VALUE);
        if (newSubCategory !== ADD_NEW_SUBCATEGORY_VALUE) {
                    ...
                    {showNewSubCategoryInput && (
                        <div className={styles.formGroup}>
                            ...
                            <input
                                ...
                                placeholder="Ej: Videojuegos, Cine, etc."
                                autoFocus
                            />
                        </div>
                    )}
